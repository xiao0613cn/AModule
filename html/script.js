...
function init() {
    ...
    m3u8 = document.getElementById("m3u8");
    if ((location.hostname != "") && (location.hostname != undefined))
	    m3u8.setAttribute("src", "http://" + location.hostname + ":8080/h264.m3u8");
    else
	m3u8.setAttribute("src", "http://localhost:8080/h264.m3u8");
...
function init_mediatypes() {
    ...
    var i = 0;
    var tr = document.createElement("tr");
    var videoTypes = [ "video/mp4", "video/webm", "video/ogg", "video/mp2t" ];
    i = 0;
...
function switchVideo(n) {
	...
	document._video.removeAttribute("src");
...
function switchM3U8()
{
	document._video.src = m3u8.getAttribute("src");
	document._video.load();
}
