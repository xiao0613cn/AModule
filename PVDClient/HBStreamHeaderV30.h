...
////////////////////////////////////////////////////////////////////////////////
// 文件名：HBStreamHeaderV30.h
// 作者：deng feng
// 版本：1.2
// 日期：2012-07-25
// 描述：定义汉邦高科第三代结构化的、可扩展的音视频帧头。
// 历史记录：
//  2011-04-20  完成汉邦高科第三代音视频帧头的定义。
//  2011-06-15  修正Stream_GetSegmentHeaderAddress函数bug。
//  2011-08-03  改写Stream_IsValidFrame函数。
//  2012-07-22  完善Stream_GenerateChecksum函数，考虑
//              STREAM_HEADER.nCheckedEncodeDataSize * 2 >= STREAM_HEADER.nEncodeDataSize
//              的情况。
//  2012-07-25  为NVR增加STREAM_ALGORITHM_VIDEO_H264_GENERAL格式。
////////////////////////////////////////////////////////////////////////////////
...
//一个典型的视频P帧（或E帧）包括：STREAM_HEADER、STREAM_VIDEO_HEADER和Video Data。
...
#define STREAM_FRAME_VIDEO_E            4           //视频E帧


//16-bit PCM只保留高8位。
//解码后为，1通道、8000Hz采样率、16-bit的PCM数据。
...
//16-bit PCM
//不需要解码，1通道、8000Hz采样率、16-bit的PCM数据。
...
#define STREAM_ALGORITHM_VIDEO_H264_GENERAL     3   //通用 H.264
...
typedef struct STREAM_HEADER
{
    ...
    //当nFrameType==STREAM_FRAME_VIDEO_*时，使用STREAM_ALGORITHM_VIDEO_*。
...
#define STREAM_TIME_FORMAT_6        6   //yyyy年M月d日 星期 HH时mm分ss秒 ==> %04d年%d月%d日 %s %02d时%02d分%02d秒
...
#define STREAM_MOSAIC_FILL_REALTIME 1               //填充实时背景色
...
typedef struct STREAM_SEGMENT_HEADER
{
    ...
    union SEGMENT_TYPE
    {
        ...
        struct OSD_STRING
        {
            ...
            unsigned int        nReserved0;         //保留
            unsigned short      nReserved1;         //保留
...
static int Stream_IsValidFrame( const void* pBuffer, unsigned int nBufferSize )
{
    ...
    if ( nBufferSize < sizeof(STREAM_HEADER) )      //判断缓冲区长度是否足够
    {
        return 0;
    }

    if ( !Stream_IsHeaderFlag( pStreamHeader ) )    //判断帧头标志
    {
        return 0;
    }
    ...
    if ( 0 == pStreamHeader->nEncodeDataSize )      //必须有编码数据
    {
        return 0;
    }
    ...
    if ( 0 == pStreamHeader->nChecksum )            //校验和不能为0
    {
        return 0;
    }
    ...
    if ( nChecksum != pStreamHeader->nChecksum )    //校验和必须相等
    {
        return 0;
    }
