...
static void PVDRelease(AObject *object)
{
	...
	release_s(pvd->outbuf, ARefsBufRelease, NULL);
}
...
extern int PVDTryOutput(DWORD userid, ARefsBuf *&outbuf, AMessage &outmsg)
{
	...
	if ((result == 0) || (result > outmsg.size)) {
		if (ARefsBufCheck(outbuf, max(result,1024), 64*1024) < 0)
			return -ENOMEM;
...
{
	pvd->outmsg.type = ioMsgType_Block;
	pvd->outmsg.data = pvd->outbuf->next();
...
static int PVDDoLogin(PVDClient *pvd, PVDStatus status)
{
	...
	const char *usr = AOptionGet((AOption*)pvd->outfrom->data, "username", "admin");
	const char *pwd = AOptionGet2((AOption*)pvd->outfrom->data, "password", "888888");
...
int PVDOpenStatus(PVDClient *pvd, int result)
{
	...
	do {
		...
		case pvdnet_connecting:
			pvd->userid = 0;
			if (ARefsBufCheck(pvd->outbuf, 2048, 64*1024) < 0) {
				result = -ENOMEM;
			} else {
...
static int PVDOpen(AObject *object, AMessage *msg)
{
	...
	pvd->io_opt = AOptionFind((AOption*)pvd->outfrom->data, "io");
	if (pvd->io == NULL) {
		int result = AObjectCreate(&pvd->io, &pvd->object, pvd->io_opt, NULL);
		if (result < 0)
			return result;
	}
...
int PVDOutputStatus(PVDClient *pvd, int result)
{
	...
	do {
		...
		if (result > 0) {
			...
			pvd->userid = phead->uUserId;
			AMsgCopy(pvd->outfrom, AMsgType_Private|phead->uCmd, pvd->outmsg.data, pvd->outmsg.size);
			break;
		}
...
static int PVDRequest(AObject *object, int reqix, AMessage *msg)
{
	...
	if (reqix != Aio_Output)
	{
		if ((reqix == Aio_Input) && (msg->type & AMsgType_Private)) {
...
int PVDCloseStatus(PVDClient *pvd, int result)
{
	...
	do {
		...
		case pvdnet_ack_logout:
			...
			if ((pvd->outfrom->type == AMsgType_Unknown) || (pvd->outfrom->type & AMsgType_Private)) {
				AMsgCopy(pvd->outfrom, AMsgType_Private|phead->uCmd, pvd->outmsg.data, pvd->outmsg.size);
			}
